...
개발 모드는 `npm run dev`입니다. 브라우저는 프론트와 동일한 출처의 `/api/v1`로 요청합니다. Next 서버가 `API_BACKEND_URL`(기본 `http://127.0.0.1:8000`)로 전달합니다. 원격 배포에서 별도 백엔드를 사용하면 이 서버 환경변수를 빌드 시 설정하세요. 브라우저용 localhost 주소를 설정할 필요가 없습니다.
